...
5. **Image Bun minimale et immuable.** Installer directement la version npm officielle exacte (`bun install -g @bitkyc08/opencodex@X.Y.Z`) sur un runtime Bun épinglé par digest. Le package contient déjà le dashboard compilé : aucun clone Git ni rebuild applicatif n'est nécessaire. Retirer seulement la copie du runtime Bun embarquée par le package lorsque l'image fournit déjà cette même version, puis exécuter en non-root avec racine read-only, `tmpfs`, capabilities supprimées, `no-new-privileges`, SBOM et scan CVE.
